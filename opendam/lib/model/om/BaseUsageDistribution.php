...
/**
 * Base class that represents a row from the 'usage_distribution' table.
 *
 * 
 *
 ...
 */
abstract class BaseUsageDistribution extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array UsageDistributionI18n[] Collection to store aggregation of UsageDistributionI18n objects.
	 */
	protected $collUsageDistributionI18ns;

	/**
	 * @var        Criteria The criteria used to select the current contents of collUsageDistributionI18ns.
	 */
	private $lastUsageDistributionI18nCriteria = null;
	...
	const PEER = 'UsageDistributionPeer';
	 ...
	 * @return     UsageDistribution The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = UsageDistributionPeer::ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = UsageDistributionPeer::CREATED_AT;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = UsageDistributionPeer::NUM_COLUMNS - UsageDistributionPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating UsageDistribution object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UsageDistributionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collUsageDistributionI18ns = null;
			$this->lastUsageDistributionI18nCriteria = null;
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseUsageDistribution:delete:pre') as $callable)
			{
			...
			if ($ret) {
				UsageDistributionPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUsageDistribution:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseUsageDistribution:save:pre') as $callable)
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(UsageDistributionPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUsageDistribution:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				UsageDistributionPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = UsageDistributionPeer::doInsert($this, $con);
					...
					$affectedRows += UsageDistributionPeer::doUpdate($this, $con);
				}
			...
			if ($this->collUsageDistributionI18ns !== null) {
				foreach ($this->collUsageDistributionI18ns as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = UsageDistributionPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = UsageDistributionPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = UsageDistributionPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsageDistributionPeer::ID)) $criteria->add(UsageDistributionPeer::ID, $this->id);
		if ($this->isColumnModified(UsageDistributionPeer::CREATED_AT)) $criteria->add(UsageDistributionPeer::CREATED_AT, $this->created_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(UsageDistributionPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of UsageDistribution (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getUsageDistributionI18ns() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addUsageDistributionI18n($relObj->copy($deepCopy));
				}
			}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     UsageDistribution Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     UsageDistributionPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new UsageDistributionPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this UsageDistribution has previously been saved, it will retrieve
	 * related Files from storage. If this UsageDistribution is new, it will return
		...
		if ($this->collFiles === null) {
			if ($this->isNew()) {
			   $this->collFiles = array();
				...
				$criteria->add(FilePeer::USAGE_DISTRIBUTION_ID, $this->id);
	 ...
	 * Otherwise if this UsageDistribution is new, it will return
	 * an empty collection; or if this UsageDistribution has previously
	 * been saved, it will retrieve related Files from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in UsageDistribution.
	 */
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this UsageDistribution has previously been saved, it will retrieve
	 * related Folders from storage. If this UsageDistribution is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collFolders === null) {
			if ($this->isNew()) {
			   $this->collFolders = array();
				...
				$criteria->add(FolderPeer::USAGE_DISTRIBUTION_ID, $this->id);
	...
	/**
	 * Clears out the collUsageDistributionI18ns collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addUsageDistributionI18ns()
	 */
	public function clearUsageDistributionI18ns()
	{
		$this->collUsageDistributionI18ns = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collUsageDistributionI18ns collection (array).
	 *
	 * By default this just sets the collUsageDistributionI18ns collection to an empty array (like clearcollUsageDistributionI18ns());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initUsageDistributionI18ns()
	{
		$this->collUsageDistributionI18ns = array();
	}

	/**
	 * Gets an array of UsageDistributionI18n objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this UsageDistribution has previously been saved, it will retrieve
	 * related UsageDistributionI18ns from storage. If this UsageDistribution is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array UsageDistributionI18n[]
	 * @throws     PropelException
	 */
	public function getUsageDistributionI18ns($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsageDistributionPeer::DATABASE_NAME);
		}
		...
		if ($this->collUsageDistributionI18ns === null) {
			if ($this->isNew()) {
			   $this->collUsageDistributionI18ns = array();
				...
				$criteria->add(UsageDistributionI18nPeer::ID, $this->id);

				UsageDistributionI18nPeer::addSelectColumns($criteria);
				$this->collUsageDistributionI18ns = UsageDistributionI18nPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				UsageDistributionI18nPeer::addSelectColumns($criteria);
				if (!isset($this->lastUsageDistributionI18nCriteria) || !$this->lastUsageDistributionI18nCriteria->equals($criteria)) {
					$this->collUsageDistributionI18ns = UsageDistributionI18nPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastUsageDistributionI18nCriteria = $criteria;
		return $this->collUsageDistributionI18ns;
	}

	/**
	 * Returns the number of related UsageDistributionI18n objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related UsageDistributionI18n objects.
	 * @throws     PropelException
	 */
	public function countUsageDistributionI18ns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsageDistributionPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collUsageDistributionI18ns === null) {
			if ($this->isNew()) {
				...
				$count = UsageDistributionI18nPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastUsageDistributionI18nCriteria) || !$this->lastUsageDistributionI18nCriteria->equals($criteria)) {
					...
					$count = count($this->collUsageDistributionI18ns);
				}
			} else {
				$count = count($this->collUsageDistributionI18ns);
			}
	...
	/**
	 * Method called to associate a UsageDistributionI18n object to this object
	 * through the UsageDistributionI18n foreign key attribute.
	 *
	 * @param      UsageDistributionI18n $l UsageDistributionI18n
	 * @return     void
	 * @throws     PropelException
	 */
	public function addUsageDistributionI18n(UsageDistributionI18n $l)
	{
		if ($this->collUsageDistributionI18ns === null) {
			$this->initUsageDistributionI18ns();
		}
		if (!in_array($l, $this->collUsageDistributionI18ns, true)) { // only add it if the **same** object is not already associated
			array_push($this->collUsageDistributionI18ns, $l);
			$l->setUsageDistribution($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this UsageDistribution has previously been saved, it will retrieve
	 * related Presets from storage. If this UsageDistribution is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collPresets === null) {
			if ($this->isNew()) {
			   $this->collPresets = array();
				...
				$criteria->add(PresetPeer::USAGE_DISTRIBUTION_ID, $this->id);
	 ...
	 * actually need in UsageDistribution.
	 */
	public function getPresetsJoinCustomer($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsageDistributionPeer::DATABASE_NAME);
		}
		...
		if ($this->collPresets === null) {
			if ($this->isNew()) {
				...
				$this->collPresets = PresetPeer::doSelectJoinCustomer($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collUsageDistributionI18ns) {
				foreach ((array) $this->collUsageDistributionI18ns as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collPresets) {
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseUsageDistribution:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseUsageDistribution::%s', $method));
	  }
	...
	/**
	 * Returns the "title" value from the current {@link UsageDistributionI18n}.
	 */
	public function getTitle($culture = null)
	{
	  return $this->getCurrentUsageDistributionI18n($culture)->getTitle();
	}
	
	/**
	 * Sets the "title" value of the current {@link UsageDistributionI18n}.
	 *
	 * @return UsageDistribution
	 */
	public function setTitle($value, $culture = null)
	{
	  $this->getCurrentUsageDistributionI18n($culture)->setTitle($value);
	  return $this;
	}
	
	/**
	 * Returns the "description" value from the current {@link UsageDistributionI18n}.
	 */
	public function getDescription($culture = null)
	{
	  return $this->getCurrentUsageDistributionI18n($culture)->getDescription();
	}
	
	/**
	 * Sets the "description" value of the current {@link UsageDistributionI18n}.
	 *
	 * @return UsageDistribution
	 */
	public function setDescription($value, $culture = null)
	{
	  $this->getCurrentUsageDistributionI18n($culture)->setDescription($value);
	  return $this;
	}
	 ...
	 */
	public function getCurrentUsageDistributionI18n($culture = null)
	{
	  if (null === $culture)
	  {
	  ...
	  {
	    $object = $this->isNew() ? null : UsageDistributionI18nPeer::retrieveByPK($this->getPrimaryKey(), $culture);
	    if ($object)
	    {
	      $this->setUsageDistributionI18nForCulture($object, $culture);
	    }
	    else
	    {
	      $this->setUsageDistributionI18nForCulture(new UsageDistributionI18n(), $culture);
	 ...
	 */
	public function setUsageDistributionI18nForCulture(UsageDistributionI18n $object, $culture)
	{
	  $this->current_i18n[$culture] = $object;
	  $this->addUsageDistributionI18n($object);
	}

} // BaseUsageDistribution
