...
/**
 * Base class that represents a row from the 'usage_commercial' table.
 *
 * 
 *
 ...
 */
abstract class BaseUsageCommercial extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array UsageCommercialI18n[] Collection to store aggregation of UsageCommercialI18n objects.
	 */
	protected $collUsageCommercialI18ns;

	/**
	 * @var        Criteria The criteria used to select the current contents of collUsageCommercialI18ns.
	 */
	private $lastUsageCommercialI18nCriteria = null;
	...
	const PEER = 'UsageCommercialPeer';
	 ...
	 * @return     UsageCommercial The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = UsageCommercialPeer::ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = UsageCommercialPeer::CREATED_AT;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = UsageCommercialPeer::NUM_COLUMNS - UsageCommercialPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating UsageCommercial object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UsageCommercialPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collUsageCommercialI18ns = null;
			$this->lastUsageCommercialI18nCriteria = null;
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseUsageCommercial:delete:pre') as $callable)
			{
			...
			if ($ret) {
				UsageCommercialPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUsageCommercial:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseUsageCommercial:save:pre') as $callable)
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(UsageCommercialPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUsageCommercial:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				UsageCommercialPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = UsageCommercialPeer::doInsert($this, $con);
					...
					$affectedRows += UsageCommercialPeer::doUpdate($this, $con);
				}
			...
			if ($this->collUsageCommercialI18ns !== null) {
				foreach ($this->collUsageCommercialI18ns as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = UsageCommercialPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = UsageCommercialPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = UsageCommercialPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsageCommercialPeer::ID)) $criteria->add(UsageCommercialPeer::ID, $this->id);
		if ($this->isColumnModified(UsageCommercialPeer::CREATED_AT)) $criteria->add(UsageCommercialPeer::CREATED_AT, $this->created_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(UsageCommercialPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of UsageCommercial (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getUsageCommercialI18ns() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addUsageCommercialI18n($relObj->copy($deepCopy));
				}
			}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     UsageCommercial Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     UsageCommercialPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new UsageCommercialPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this UsageCommercial has previously been saved, it will retrieve
	 * related Files from storage. If this UsageCommercial is new, it will return
		...
		if ($this->collFiles === null) {
			if ($this->isNew()) {
			   $this->collFiles = array();
				...
				$criteria->add(FilePeer::USAGE_COMMERCIAL_ID, $this->id);
	 ...
	 * Otherwise if this UsageCommercial is new, it will return
	 * an empty collection; or if this UsageCommercial has previously
	 * been saved, it will retrieve related Files from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in UsageCommercial.
	 */
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this UsageCommercial has previously been saved, it will retrieve
	 * related Folders from storage. If this UsageCommercial is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collFolders === null) {
			if ($this->isNew()) {
			   $this->collFolders = array();
				...
				$criteria->add(FolderPeer::USAGE_COMMERCIAL_ID, $this->id);
	...
	/**
	 * Clears out the collUsageCommercialI18ns collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addUsageCommercialI18ns()
	 */
	public function clearUsageCommercialI18ns()
	{
		$this->collUsageCommercialI18ns = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collUsageCommercialI18ns collection (array).
	 *
	 * By default this just sets the collUsageCommercialI18ns collection to an empty array (like clearcollUsageCommercialI18ns());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initUsageCommercialI18ns()
	{
		$this->collUsageCommercialI18ns = array();
	}

	/**
	 * Gets an array of UsageCommercialI18n objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this UsageCommercial has previously been saved, it will retrieve
	 * related UsageCommercialI18ns from storage. If this UsageCommercial is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array UsageCommercialI18n[]
	 * @throws     PropelException
	 */
	public function getUsageCommercialI18ns($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsageCommercialPeer::DATABASE_NAME);
		}
		...
		if ($this->collUsageCommercialI18ns === null) {
			if ($this->isNew()) {
			   $this->collUsageCommercialI18ns = array();
				...
				$criteria->add(UsageCommercialI18nPeer::ID, $this->id);

				UsageCommercialI18nPeer::addSelectColumns($criteria);
				$this->collUsageCommercialI18ns = UsageCommercialI18nPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				UsageCommercialI18nPeer::addSelectColumns($criteria);
				if (!isset($this->lastUsageCommercialI18nCriteria) || !$this->lastUsageCommercialI18nCriteria->equals($criteria)) {
					$this->collUsageCommercialI18ns = UsageCommercialI18nPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastUsageCommercialI18nCriteria = $criteria;
		return $this->collUsageCommercialI18ns;
	}

	/**
	 * Returns the number of related UsageCommercialI18n objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related UsageCommercialI18n objects.
	 * @throws     PropelException
	 */
	public function countUsageCommercialI18ns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsageCommercialPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collUsageCommercialI18ns === null) {
			if ($this->isNew()) {
				...
				$count = UsageCommercialI18nPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastUsageCommercialI18nCriteria) || !$this->lastUsageCommercialI18nCriteria->equals($criteria)) {
					...
					$count = count($this->collUsageCommercialI18ns);
				}
			} else {
				$count = count($this->collUsageCommercialI18ns);
			}
	...
	/**
	 * Method called to associate a UsageCommercialI18n object to this object
	 * through the UsageCommercialI18n foreign key attribute.
	 *
	 * @param      UsageCommercialI18n $l UsageCommercialI18n
	 * @return     void
	 * @throws     PropelException
	 */
	public function addUsageCommercialI18n(UsageCommercialI18n $l)
	{
		if ($this->collUsageCommercialI18ns === null) {
			$this->initUsageCommercialI18ns();
		}
		if (!in_array($l, $this->collUsageCommercialI18ns, true)) { // only add it if the **same** object is not already associated
			array_push($this->collUsageCommercialI18ns, $l);
			$l->setUsageCommercial($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this UsageCommercial has previously been saved, it will retrieve
	 * related Presets from storage. If this UsageCommercial is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collPresets === null) {
			if ($this->isNew()) {
			   $this->collPresets = array();
				...
				$criteria->add(PresetPeer::USAGE_COMMERCIAL_ID, $this->id);
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collUsageCommercialI18ns) {
				foreach ((array) $this->collUsageCommercialI18ns as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collPresets) {
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseUsageCommercial:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseUsageCommercial::%s', $method));
	  }
	...
	/**
	 * Returns the "title" value from the current {@link UsageCommercialI18n}.
	 */
	public function getTitle($culture = null)
	{
	  return $this->getCurrentUsageCommercialI18n($culture)->getTitle();
	}
	
	/**
	 * Sets the "title" value of the current {@link UsageCommercialI18n}.
	 *
	 * @return UsageCommercial
	 */
	public function setTitle($value, $culture = null)
	{
	  $this->getCurrentUsageCommercialI18n($culture)->setTitle($value);
	  return $this;
	}
	 ...
	 */
	public function getCurrentUsageCommercialI18n($culture = null)
	{
	  if (null === $culture)
	  {
	  ...
	  {
	    $object = $this->isNew() ? null : UsageCommercialI18nPeer::retrieveByPK($this->getPrimaryKey(), $culture);
	    if ($object)
	    {
	      $this->setUsageCommercialI18nForCulture($object, $culture);
	    }
	    else
	    {
	      $this->setUsageCommercialI18nForCulture(new UsageCommercialI18n(), $culture);
	 ...
	 */
	public function setUsageCommercialI18nForCulture(UsageCommercialI18n $object, $culture)
	{
	  $this->current_i18n[$culture] = $object;
	  $this->addUsageCommercialI18n($object);
	}
