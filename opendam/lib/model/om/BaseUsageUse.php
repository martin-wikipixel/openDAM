...
/**
 * Base class that represents a row from the 'usage_use' table.
 *
 * 
 *
 ...
 */
abstract class BaseUsageUse extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array UsageUseI18n[] Collection to store aggregation of UsageUseI18n objects.
	 */
	protected $collUsageUseI18ns;

	/**
	 * @var        Criteria The criteria used to select the current contents of collUsageUseI18ns.
	 */
	private $lastUsageUseI18nCriteria = null;
	 ...
	 * @return     UsageUse The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = UsageUsePeer::ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = UsageUsePeer::CREATED_AT;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = UsageUsePeer::NUM_COLUMNS - UsageUsePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating UsageUse object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UsageUsePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collUsageUseI18ns = null;
			$this->lastUsageUseI18nCriteria = null;
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseUsageUse:delete:pre') as $callable)
			{
			...
			if ($ret) {
				UsageUsePeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUsageUse:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseUsageUse:save:pre') as $callable)
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(UsageUsePeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUsageUse:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				UsageUsePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = UsageUsePeer::doInsert($this, $con);
					...
					$affectedRows += UsageUsePeer::doUpdate($this, $con);
				}
			...
			if ($this->collUsageUseI18ns !== null) {
				foreach ($this->collUsageUseI18ns as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = UsageUsePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = UsageUsePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = UsageUsePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsageUsePeer::ID)) $criteria->add(UsageUsePeer::ID, $this->id);
		if ($this->isColumnModified(UsageUsePeer::CREATED_AT)) $criteria->add(UsageUsePeer::CREATED_AT, $this->created_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(UsageUsePeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of UsageUse (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getUsageUseI18ns() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addUsageUseI18n($relObj->copy($deepCopy));
				}
			}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     UsageUse Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this UsageUse has previously been saved, it will retrieve
	 * related Files from storage. If this UsageUse is new, it will return
		...
		if ($this->collFiles === null) {
			if ($this->isNew()) {
			   $this->collFiles = array();
				...
				$criteria->add(FilePeer::USAGE_USE_ID, $this->id);
	 ...
	 * Otherwise if this UsageUse is new, it will return
	 * an empty collection; or if this UsageUse has previously
	 * been saved, it will retrieve related Files from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in UsageUse.
	 */
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this UsageUse has previously been saved, it will retrieve
	 * related Folders from storage. If this UsageUse is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collFolders === null) {
			if ($this->isNew()) {
			   $this->collFolders = array();
				...
				$criteria->add(FolderPeer::USAGE_USE_ID, $this->id);
	...
	/**
	 * Clears out the collUsageUseI18ns collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addUsageUseI18ns()
	 */
	public function clearUsageUseI18ns()
	{
		$this->collUsageUseI18ns = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collUsageUseI18ns collection (array).
	 *
	 * By default this just sets the collUsageUseI18ns collection to an empty array (like clearcollUsageUseI18ns());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initUsageUseI18ns()
	{
		$this->collUsageUseI18ns = array();
	}

	/**
	 * Gets an array of UsageUseI18n objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this UsageUse has previously been saved, it will retrieve
	 * related UsageUseI18ns from storage. If this UsageUse is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array UsageUseI18n[]
	 * @throws     PropelException
	 */
	public function getUsageUseI18ns($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsageUsePeer::DATABASE_NAME);
		}
		...
		if ($this->collUsageUseI18ns === null) {
			if ($this->isNew()) {
			   $this->collUsageUseI18ns = array();
				...
				$criteria->add(UsageUseI18nPeer::ID, $this->id);

				UsageUseI18nPeer::addSelectColumns($criteria);
				$this->collUsageUseI18ns = UsageUseI18nPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				UsageUseI18nPeer::addSelectColumns($criteria);
				if (!isset($this->lastUsageUseI18nCriteria) || !$this->lastUsageUseI18nCriteria->equals($criteria)) {
					$this->collUsageUseI18ns = UsageUseI18nPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastUsageUseI18nCriteria = $criteria;
		return $this->collUsageUseI18ns;
	}

	/**
	 * Returns the number of related UsageUseI18n objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related UsageUseI18n objects.
	 * @throws     PropelException
	 */
	public function countUsageUseI18ns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsageUsePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collUsageUseI18ns === null) {
			if ($this->isNew()) {
				...
				$count = UsageUseI18nPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastUsageUseI18nCriteria) || !$this->lastUsageUseI18nCriteria->equals($criteria)) {
					...
					$count = count($this->collUsageUseI18ns);
				}
			} else {
				$count = count($this->collUsageUseI18ns);
			}
	...
	/**
	 * Method called to associate a UsageUseI18n object to this object
	 * through the UsageUseI18n foreign key attribute.
	 *
	 * @param      UsageUseI18n $l UsageUseI18n
	 * @return     void
	 * @throws     PropelException
	 */
	public function addUsageUseI18n(UsageUseI18n $l)
	{
		if ($this->collUsageUseI18ns === null) {
			$this->initUsageUseI18ns();
		}
		if (!in_array($l, $this->collUsageUseI18ns, true)) { // only add it if the **same** object is not already associated
			array_push($this->collUsageUseI18ns, $l);
			$l->setUsageUse($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this UsageUse has previously been saved, it will retrieve
	 * related Presets from storage. If this UsageUse is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collPresets === null) {
			if ($this->isNew()) {
			   $this->collPresets = array();
				...
				$criteria->add(PresetPeer::USAGE_USE_ID, $this->id);
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collUsageUseI18ns) {
				foreach ((array) $this->collUsageUseI18ns as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collPresets) {
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseUsageUse:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseUsageUse::%s', $method));
	  }
	...
	/**
	 * Returns the "title" value from the current {@link UsageUseI18n}.
	 */
	public function getTitle($culture = null)
	{
	  return $this->getCurrentUsageUseI18n($culture)->getTitle();
	}
	
	/**
	 * Sets the "title" value of the current {@link UsageUseI18n}.
	 *
	 * @return UsageUse
	 */
	public function setTitle($value, $culture = null)
	{
	  $this->getCurrentUsageUseI18n($culture)->setTitle($value);
	  return $this;
	}
	
	/**
	 * Returns the "description" value from the current {@link UsageUseI18n}.
	 */
	public function getDescription($culture = null)
	{
	  return $this->getCurrentUsageUseI18n($culture)->getDescription();
	}
	
	/**
	 * Sets the "description" value of the current {@link UsageUseI18n}.
	 *
	 * @return UsageUse
	 */
	public function setDescription($value, $culture = null)
	{
	  $this->getCurrentUsageUseI18n($culture)->setDescription($value);
	  return $this;
	}
	 ...
	 * @return UsageUseI18n
	 */
	public function getCurrentUsageUseI18n($culture = null)
	{
	  if (null === $culture)
	  {
	  ...
	  {
	    $object = $this->isNew() ? null : UsageUseI18nPeer::retrieveByPK($this->getPrimaryKey(), $culture);
	    if ($object)
	    {
	      $this->setUsageUseI18nForCulture($object, $culture);
	    }
	    else
	    {
	      $this->setUsageUseI18nForCulture(new UsageUseI18n(), $culture);
	 ...
	 */
	public function setUsageUseI18nForCulture(UsageUseI18n $object, $culture)
	{
	  $this->current_i18n[$culture] = $object;
	  $this->addUsageUseI18n($object);
	}
