...
abstract class BaseCountryI18nPeer {
	...
	const CLASS_DEFAULT = 'lib.model.CountryI18n';
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseCountryI18nPeer', $criteria, $con);
		}
	 ...
	 *
	 * @param      CountryI18n $value A CountryI18n object.
	...
	/**
	 * Method perform an INSERT on the database, given a CountryI18n or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseCountryI18nPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseCountryI18nPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseCountryI18nPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseCountryI18nPeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a CountryI18n or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseCountryI18nPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseCountryI18nPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseCountryI18nPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseCountryI18nPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the country_i18n table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	/**
	 * Method perform a DELETE on the database, given a CountryI18n or Criteria object OR a primary key value.
	 *
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseCountryI18nPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
