...
abstract class BaseEmailPeer {
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(EmailPeer::NAME);
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseEmailPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseEmailPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseEmailPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseEmailPeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseEmailPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseEmailPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseEmailPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseEmailPeer', $values, $con, $ret);
    }
	...
	/**
	 * Selects a collection of {@link Email} objects with a {@link EmailI18n} translation populated.
	 *
	 * @param Criteria  $criteria
	...
	static public function doSelectWithI18n(Criteria $criteria, $culture = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
	  ...
	  $criteria->addJoin(EmailPeer::ID, EmailI18nPeer::ID, $join_behavior);
	  $criteria->add(EmailI18nPeer::CULTURE, $culture);
	
	  foreach (sfMixer::getCallables('BaseEmail:doSelectJoin:doSelectJoin') as $sf_hook)
	  {
	    call_user_func($sf_hook, 'Email', $criteria, $con);
	  }
	  ...
	  $stmt = BasePeer::doSelect($criteria, $con);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
	      ...
	      EmailPeer::addInstanceToPool($obj1, $key1);
			...
			$key2 = EmailI18nPeer::getPrimaryKeyHashFromRow($row, $startcol);
			if ($key2 !== null) {
				$obj2 = EmailI18nPeer::getInstanceFromPool($key2);
				if (!$obj2) {
					$cls = EmailI18nPeer::getOMClass(false);
					$obj2 = new $cls();
					$obj2->hydrate($row, $startcol);
					EmailI18nPeer::addInstanceToPool($obj2, $key2);
				} // if obj2 already loaded
	
	      $obj1->setEmailI18nForCulture($obj2, $culture);
