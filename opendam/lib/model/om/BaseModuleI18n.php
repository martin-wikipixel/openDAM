...
abstract class BaseModuleI18n extends BaseObject  implements Persistent {
	...
	public function setCulture($v)
	{
		...
		if ($this->culture !== $v) {
			$this->culture = $v;
			$this->modifiedColumns[] = ModuleI18nPeer::CULTURE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = ModuleI18nPeer::NUM_COLUMNS - ModuleI18nPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseModuleI18n:delete:pre') as $callable)
			{
			...
			if ($ret) {
				ModuleI18nPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseModuleI18n:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseModuleI18n:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseModuleI18n:save:post') as $callable)
				{
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aModule !== null) {
				if ($this->aModule->isModified() || ($this->aModule->getCulture() && $this->aModule->getCurrentModuleI18n()->isModified()) || $this->aModule->isNew()) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ModuleI18nPeer::doInsert($this, $con);
					...
					$affectedRows += ModuleI18nPeer::doUpdate($this, $con);
				}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ModuleI18nPeer::CULTURE)) $criteria->add(ModuleI18nPeer::CULTURE, $this->culture);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ModuleI18nPeer::CULTURE, $this->culture);
	...
	public function getModule(PropelPDO $con = null)
	{
		if ($this->aModule === null && ($this->id !== null)) {
			$this->aModule = ModulePeer::retrieveByPk($this->id);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseModuleI18n:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseModuleI18n::%s', $method));
	  }
