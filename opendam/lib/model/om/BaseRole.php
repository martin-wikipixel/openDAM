...
abstract class BaseRole extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Constraint[] Collection to store aggregation of Constraint objects.
	 */
	protected $collConstraints;

	/**
	 * @var        Criteria The criteria used to select the current contents of collConstraints.
	 */
	private $lastConstraintCriteria = null;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = RolePeer::NUM_COLUMNS - RolePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collConstraints = null;
			$this->lastConstraintCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collConstraints !== null) {
				foreach ($this->collConstraints as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getConstraints() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addConstraint($relObj->copy($deepCopy));
				}
			}
	 ...
	 * related Groupes from storage. If this Role is new, it will return
		...
		if ($this->collGroupes === null) {
			if ($this->isNew()) {
			   $this->collGroupes = array();
				...
				$criteria->add(GroupePeer::FREE_CREDENTIAL, $this->id);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Role has previously been saved, it will retrieve
	 * related UnitGroups from storage. If this Role is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collUnitGroups === null) {
			if ($this->isNew()) {
			   $this->collUnitGroups = array();
				...
				$criteria->add(UnitGroupPeer::ROLE, $this->id);
	...
	/**
	 * Clears out the collConstraints collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	...
	{
		$this->collConstraints = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collConstraints collection (array).
	 *
	 * By default this just sets the collConstraints collection to an empty array (like clearcollConstraints());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	...
	{
		$this->collConstraints = array();
	}

	/**
	 * Gets an array of Constraint objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Role has previously been saved, it will retrieve
	 * related Constraints from storage. If this Role is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getConstraints($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(RolePeer::DATABASE_NAME);
		}
		...
		if ($this->collConstraints === null) {
			if ($this->isNew()) {
			   $this->collConstraints = array();
				...
				$criteria->add(ConstraintPeer::ROLE_ID, $this->id);

				ConstraintPeer::addSelectColumns($criteria);
				$this->collConstraints = ConstraintPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				ConstraintPeer::addSelectColumns($criteria);
				if (!isset($this->lastConstraintCriteria) || !$this->lastConstraintCriteria->equals($criteria)) {
					$this->collConstraints = ConstraintPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastConstraintCriteria = $criteria;
		return $this->collConstraints;
	}

	/**
	 * Returns the number of related Constraint objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Constraint objects.
	 * @throws     PropelException
	 */
	public function countConstraints(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(RolePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collConstraints === null) {
			if ($this->isNew()) {
				...
				$count = ConstraintPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastConstraintCriteria) || !$this->lastConstraintCriteria->equals($criteria)) {
					...
					$count = count($this->collConstraints);
				}
			} else {
				$count = count($this->collConstraints);
			}
	...
	/**
	 * Method called to associate a Constraint object to this object
	 * through the Constraint foreign key attribute.
	 *
	 * @param      Constraint $l Constraint
	 * @return     void
	 * @throws     PropelException
	 */
	public function addConstraint(Constraint $l)
	{
		if ($this->collConstraints === null) {
			$this->initConstraints();
		}
		if (!in_array($l, $this->collConstraints, true)) { // only add it if the **same** object is not already associated
			array_push($this->collConstraints, $l);
			$l->setRole($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collConstraints) {
				foreach ((array) $this->collConstraints as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
