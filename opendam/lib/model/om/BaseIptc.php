...
/**
 * Base class that represents a row from the 'iptc' table.
 *
 * 
 *
...
abstract class BaseIptc extends BaseObject  implements Persistent {
	 ...
	 * @return     Iptc The current object (for fluent API support)
	 */
	...
	public function setTitle($v)
	{
		...
		if ($this->title !== $v) {
			$this->title = $v;
			$this->modifiedColumns[] = IptcPeer::TITLE;
		}
	...
	public function setValue($v)
	{
		...
		if ($this->value !== $v) {
			$this->value = $v;
			$this->modifiedColumns[] = IptcPeer::VALUE;
		}
	...
	public function setFileId($v)
	{
		...
		if ($this->file_id !== $v) {
			$this->file_id = $v;
			$this->modifiedColumns[] = IptcPeer::FILE_ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = IptcPeer::CREATED_AT;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = IptcPeer::NUM_COLUMNS - IptcPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = IptcPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseIptc:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseIptc:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseIptc:save:pre') as $callable)
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(IptcPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseIptc:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				IptcPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(IptcPeer::TITLE)) $criteria->add(IptcPeer::TITLE, $this->title);
		if ($this->isColumnModified(IptcPeer::VALUE)) $criteria->add(IptcPeer::VALUE, $this->value);
		if ($this->isColumnModified(IptcPeer::FILE_ID)) $criteria->add(IptcPeer::FILE_ID, $this->file_id);
		if ($this->isColumnModified(IptcPeer::CREATED_AT)) $criteria->add(IptcPeer::CREATED_AT, $this->created_at);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Iptc (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Iptc Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseIptc:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseIptc::%s', $method));
	  }
