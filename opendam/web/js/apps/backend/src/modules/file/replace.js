(function($) {
	...
	$(document).ready(function() {
		...
		var fileId = $upload.attr("data-file-id");
		...
		{
			if (dataArray.length > 0) {
				notification.error(__("You must select one file."));
				return;
			}
		...
		function displayFile(file, image)
		{
			...
			$newFile.find("button").on("click", function() {
				...
				if (dataArray.length == 0 && $btnUpload.is(":visible")) {
		...
		function mergeFile(html, file, name)
		{
			$.ajax({
				type: "POST",
				url: Routing.generate("upload_file_replace"), 
				data: {"name": name, "originalName": file.name, "index": slices2, "keyId": keyId, "fileId": fileId}
			})
			.done(function() {
