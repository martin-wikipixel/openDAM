(function($) {
	...
	$(document).ready(function() {
		var $root = $("#right-guest-list-page");
		...
		$permissionsTable.find("tbody [data-action=notify-user]").on("click", function() {
			...
			$.ajax(Routing.generate("group_right_guest_notify", {"album": albumId, "guest": guestId}))
		...
		$permissionsTable.find("tbody input[type=radio]").on("change", function() {
			...
			$.ajax(Routing.generate("group_right_guest_update", {"album": albumId, "guest": guestId}), {
				data: {"role": roleId}
			})
			.done(function(){
