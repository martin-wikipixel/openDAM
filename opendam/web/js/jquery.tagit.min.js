...
(function($) {
    $.widget("ui.tagit", {
        ...
        _create: function() {
            ...
            this.options.select = function(event, ui) {
                ...
                if (ui.item.label === undefined)
                	...
                	self._addTag(ui.item.label, ui.item.id, "autocomplete");
                return false;
            }
            ...
            this.options.focus = function(event, ui) {
            	if (ui.item.label !== undefined && /^key/.test(event.originalEvent.originalEvent.type)) {
            		...
            		inputBox.attr('tagValue', ui.item.id);
            		return false;
            	}
            }
            ...
            this.input.keyup(function(e){
				if(!($.browser.msie && $.browser.version <= 7))
					$(this).attr({width: 'auto', size: ($(this).val().length + self._confort_zone)});
            ...
            this.input.blur(function(e) {
                self.currentLabel = $(this).val();
                self.currentValue = $(this).attr('tagValue');
                if(self.options.allowNewTags && self.options.saveOnblur) {
                    self.timer = setTimeout(function(){
                        self._addTag(self.currentLabel, self.currentValue);
                        self.currentValue = '';
                        self.currentLabel = '';
				...
				$(this).bind("focus", function(event) {
					$(this).removeClass(self.options.initialClass);
				});
        ...
        _addTag: function(label, value, type) {
			this.input.val("");
			...
			if(this.options.checkEmail == true && !this._email_pattern.test(label) && type != "autocomplete")
				return false;
		...
		_initialText: function() {
			var input = this;
			if (this.options.initialText.length != 0) {
				this.input.val(this.options.initialText);

				if(this.options.initialClass.length != 0)
					this.input.addClass(this.options.initialClass);

				this.input.bind("focus", function(event) {
					if ($(this).val() == input.options.initialText && input.tagsArray.length == 0) {
						$(this).val("");
						$(this).removeClass(input.options.initialClass);
					}
				});

				this.input.bind("blur", function(event) {
					if ($(this).val() == "" && input.tagsArray.length == 0) {
						$(this).val(input.options.initialText);
						$(this).addClass(input.options.initialClass);
					}
				});
			}
		}
